...
class Graph:
    def __init__(self, input_data, data_type=False):  # geo is False
        ...
        self._create_adj_matrix(input_data, data_type)
        self._distances = {}  # distance dict key its a tuple (v, s)
        self._predecessors = {}  # again a tuple key

    def _create_adj_matrix(self, input_data, data_type):  # format, pass data_type o euc format
        for i in input_data:
            for j in input_data:
                if i[0] != j[0]:
                    # geo = False
                    self._adj_matrix[int(i[0]) - 1][int(j[0]) - 1] = calc_distance(i, j, data_type)  # None if i == j
...
def calc_distance(i, j, geo):
    if geo:
        lat_i = convert_to_rads(i[1])  # u[0] index, u[1] node latitude
